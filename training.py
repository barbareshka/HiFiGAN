...
def train(a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_disc_f, losses_disc_f_r, losses_disc_f_g = d_loss(y_df_hat_r, y_df_hat_g)

            # MSD
            y_ds_hat_r, y_ds_hat_g, _, _ = msd(y, y_g_hat.detach())
            loss_disc_s, losses_disc_s_r, losses_disc_s_g = d_loss(y_ds_hat_r, y_ds_hat_g)
            ...
            loss_fm_f = f_loss(fmap_f_r, fmap_f_g)
            loss_fm_s = f_loss(fmap_s_r, fmap_s_g)
            loss_gen_f, losses_gen_f = g_loss(y_df_hat_g)
            loss_gen_s, losses_gen_s = g_loss(y_ds_hat_g)
